...
- [Jenpo/dgx-spark-monitoring](https://github.com/Jenpo/dgx-spark-monitoring) - One-command DGX Spark (GB10) cluster monitoring with a 17-panel Grafana dashboard: 14 generic DCGM/node/vLLM panels plus 3 GB10-specific ones — unified 128 GB CPU+GPU memory (no separate frame-buffer), SM clock vs the 2200 MHz software clock-cap (3003 MHz silicon max), and power–temperature co-plot for the cap's thermal trade-off — multi-node over the 200 Gb/s fabric.
